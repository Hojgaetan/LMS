...
def add_member():
    try:
        ...
        if existing_member:
            return jsonify({"success": False, "message": "Member with this email already exist."}), 400
...
def update_member():
    try:
        ...
        if not member:
            return (
                jsonify(
                    {"success": False, "message": "Le membre que vous essayez de modifier n'existe pas dans la base de données."}
                ),
                404,
            )
        ...
        if missing_fields:
            ...
            return (
                jsonify(
                    {"success": False, "message": f'Veuillez remplir tous les champs obligatoires : {", ".join(missing_fields)}'}
                ),
                400,
            )
        ...
        if success:
            return (
                jsonify({"success": True, "message": f'Les informations de {data.get("name")} ont été mises à jour avec succès.'}),
                200,
            )
        else:
            return jsonify({"success": False, "message": msg}), 400
...
def delete_member():
    try:
        ...
        if not data or "member_id" not in data:
            return jsonify({"success": False, "message": "ID du membre manquant dans la requête"}), 400

        member_id = data.get("member_id")
        if not isinstance(member_id, (int, str)) or not str(member_id).isdigit():
            return jsonify({"success": False, "message": "ID du membre invalide"}), 400
        ...
        member = Member.find_by_id(member_id)
        if not member:
            return jsonify({"success": False, "message": f"Membre avec ID {member_id} non trouvé"}), 404
        ...
        if active_borrowings:
            return jsonify({"success": False, "message": "Impossible de supprimer ce membre car il a des emprunts en cours"}), 400

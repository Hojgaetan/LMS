class BookView:
    ...
    async def get_book_details():
        ...
        try:
            ...
            publisher = await aio.async_input("Publisher (optional, press Enter to skip): ")
            if publisher:
    ...
    @staticmethod
    async def get_book_id_for_update():
        """
        Get the ID of the book to update.

        Returns:
            int or None: The book ID, or None if the await aio.async_input is invalid
        """
        try:
            book_id = int(await aio.async_input("Enter the ID of the book to update: "))
            return book_id
        except ValueError:
    ...
    @staticmethod
    async def get_book_update_details():
        """
        ...
        # Get optional fields
        title = await aio.async_input("Title: ")
        if title:
            book_updates["title"] = title

        try:
            author_id = await aio.async_input("Author ID: ")
            if author_id:
                book_updates["author_id"] = int(author_id)

            category_id = await aio.async_input("Category ID: ")
            if category_id:
                book_updates["category_id"] = int(category_id)

            isbn = await aio.async_input("ISBN: ")
            if isbn:
                book_updates["isbn"] = isbn

            pub_year = await aio.async_input("Publication Year: ")
            if pub_year:
                book_updates["publication_year"] = int(pub_year)

            publisher = await aio.async_input("Publisher: ")
            if publisher:
                book_updates["publisher"] = publisher

            quantity = await aio.async_input("Quantity: ")
            if quantity:
    ...
    @staticmethod
    async def remove_book_menu():
        """
        Display the menu for removing a book and handle user await aio.async_input.
        """
        print("\n===== Remove Book =====")
        try:
            book_id = int(await aio.async_input("Enter the Book ID to remove: "))
        except ValueError:
            print("Invalid await aio.async_input. Please enter a valid Book ID.")
            return

...
class LibraryManagementSystem:
    ...
    def __init__(self):
        ...
        # Services (static-like usage, but instances kept for clarity / possible future state)
    ...
    def add_new_book(self):
        ...
        authors = self.author_service.list_authors() if hasattr(self.author_service, "list_authors") else self.author_service.get_all_authors()
    ...
    def update_book(self):
        ...
        # FIX: method was get_all_categories() which does not exist in CategoryService
        categories = self.category_service.list_categories()
        self.book_view.display_categories(categories)
    ...
    def remove_book(self):
        """Handle the remove book functionality (delegated to view + service)."""
    ...
    def add_new_author(self):
        ...
        success, message = self.author_service.add_author(name, biography)
    ...
    def update_author(self):
        ...
        if not author_id.isdigit():
            ...
            return
        success, message = self.author_service.update_author(int(author_id), name or None, biography or None)
        AuthorView.display_message(message)
    ...
    def delete_author(self):
        ...
        if not author_id.isdigit():
            AuthorView.display_message("Invalid author ID.")
            return
        success, message = self.author_service.delete_author(int(author_id))
        AuthorView.display_message(message)

    def list_authors(self):
        authors = self.author_service.list_authors()
        AuthorView.display_authors(authors)

    def search_authors(self):
        name = AuthorView.display_search_author_menu()
        authors = self.author_service.search_authors_by_name(name)
        AuthorView.display_authors(authors)

    # -------------------- CATEGORIES -------------------- #
    def add_new_category(self):
        name, description = CategoryView.display_add_category_menu()
        success, message = self.category_service.add_category(name, description)
    ...
    def update_category(self):
        ...
        if not category_id.isdigit():
            ...
            return
        success, message = self.category_service.update_category(int(category_id), name or None, description or None)
        CategoryView.display_message(message)
    ...
    def delete_category(self):
        ...
        if not category_id.isdigit():
            CategoryView.display_message("Invalid category ID.")
            return
        success, message = self.category_service.delete_category(int(category_id))
        CategoryView.display_message(message)
    ...
    def search_categories(self):
        ...
        categories = self.category_service.search_categories_by_name(name)
        CategoryView.display_categories(categories)

    # -------------------- MEMBERS -------------------- #
    def add_new_member(self):
        name, email, phone, address = MemberView.display_add_member_menu()
        success, message = self.member_service.add_member(name, email, phone, address)
    ...
    def update_member(self):
        ...
        if not member_id.isdigit():
            ...
            return
        success, message = self.member_service.update_member(
    ...
    def delete_member(self):
        ...
        if not member_id.isdigit():
            MemberView.display_message("Invalid member ID.")
            return
        success, message = self.member_service.delete_member(int(member_id))
        MemberView.display_message(message)
    ...
    def search_members(self):
        ...
        members = self.member_service.search_members_by_name(name)
        MemberView.display_members(members)
    ...
    def view_member_history(self):
        ...
        if not member_id.isdigit():
            MemberView.display_message("Invalid member ID.")
            return
        history = self.member_service.get_member_history(int(member_id))
    ...
    def set_member_status(self):
        ...
        success, message = self.member_service.set_member_status(int(member_id), status)
        MemberView.display_message(message)
    ...
    def borrow_book(self):
        ...
        if not book_id.isdigit() or not member_id.isdigit():
            ...
            return
        success, message = self.borrowing_service.borrow_book(int(book_id), int(member_id), days)
    ...
    def return_book(self):
        ...
        if not borrowing_id.isdigit():
            ...
            return
        success, message = self.borrowing_service.return_book(int(borrowing_id))
        BorrowingView.display_message(message)
    ...
    def extend_borrowing(self):
        ...
        if not borrowing_id.isdigit() or extra_days <= 0:
            ...
            return
        success, message = self.borrowing_service.extend_borrowing(int(borrowing_id), extra_days)
        BorrowingView.display_message(message)

    def show_overdue_borrowings(self):
        overdues = self.borrowing_service.get_overdue_borrowings()
    ...
    def send_borrowing_reminder(self):
        ...
        if not borrowing_id.isdigit():
            BorrowingView.display_message("Invalid borrowing ID.")
            return
        success, message = self.borrowing_service.send_reminder(int(borrowing_id))
        BorrowingView.display_message(message)

    # -------------------- ADMIN -------------------- #
    def admin_login(self):
        username, password = AdminView.display_login_menu()
        success, message = self.admin_service.authenticate_user(username, password)
        AdminView.display_message(message)
        return success

    def set_system_config(self):
        key, value = AdminView.display_config_menu()
        success, message = self.admin_service.set_config(key, value)
        AdminView.display_message(message)

    def backup_database(self):
        path = AdminView.display_backup_menu()
        success, message = self.admin_service.backup_database(path)
        AdminView.display_message(message)

    def restore_database(self):
        path = AdminView.display_restore_menu()
        success, message = self.admin_service.restore_database(path)
        AdminView.display_message(message)

    def log_activity(self, action, user="system"):
        self.admin_service.log_activity(action, user)
